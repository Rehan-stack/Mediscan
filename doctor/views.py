...
from .models import Doctor_Information, Appointment, Education, Experience, Prescription_medicine, Report,Test, Prescription_test, Prescription, Doctor_review
from hospital_admin.models import Admin_Information,Clinical_Laboratory_Technician, Test_Information
from .models import Doctor_Information, Appointment, Education, Experience, Prescription_medicine, Report,Test, Prescription_test, Prescription
...
media = 'media'
model = keras.models.load_model('D:\django projects\FYP\HealthStack-System-main\HealthStack-System-main\doctor\lungs_cancer.h5')
model2 = keras.models.load_model('D:\django projects\FYP\HealthStack-System-main\HealthStack-System-main\doctor\model.h5')
model3 = keras.models.load_model('D:\django projects\FYP\HealthStack-System-main\HealthStack-System-main\doctor\DN21_model.h5')

import requests
from transformers import GPT2LMHeadModel, GPT2Tokenizer
model_name = "gpt2"  # You can use other variants like "gpt2-medium" for different model sizes
tokenizer = GPT2Tokenizer.from_pretrained(model_name)
model4 = GPT2LMHeadModel.from_pretrained(model_name)
# You can use other variants like "gpt2-medium" for different model sizes
...
def doctor_dashboard(request):
        if request.user.is_authenticated:    
            ...
            context = {'doctor': doctor, 'today_appointments': today_appointments, 'today_patient_count': today_patient_count, 'total_appointments_count': total_appointments_count, 'next_days_appointment': next_days_appointment, 'current_date': current_date_str, 'next_date': next_date_str,'appointments':appointments}
...
def appointments(request):
    ...
    # Render HTML for the updated appointments
    upcoming_appointments_html = render_to_string('upcoming_appointments_partial.html', {'appointments': appointments})
    today_appointments_html = render_to_string('today_appointments_partial.html', {'appointments': appointments})

    # Send the rendered HTML to the WebSocket group
    ...
    async_to_sync(channel_layer.group_send)(
        'appointments',
        {
            'type': 'send_update',
            'type': 'upcoming',
            'html': upcoming_appointments_html
        }
    )
    async_to_sync(channel_layer.group_send)(
        'appointments',
...
def accept_appointment(request, pk):
    ...
    try:
        send_mail(subject, plain_message, 'chaule713@gmail.com',  [patient_email], html_message=html_message, fail_silently=False)
...
def report_pdf(request, pk):
 if request.user.is_patient:
    ...
    # current_date = datetime.date.today()
    context={'patient':patient,'report':report,'test':test}
...
def doctor_view_report(request, pk):
    if request.user.is_authenticated and request.user.is_doctor:
        ...
        test = Test.objects.filter(report=report)
        context = {'report': report, 'test': test,'doctor': doctor}
...
def fracture_predict(path):
    ...
    predictions = model2.predict(rgb_img)
    predicted_class_index = np.argmax(predictions[0])

    a = int(np.argmax(predictions))
    predicted_class = class_names_dict[a]
  

    # if a==2:
    #  a = "No Disease Detected."
    # elif a==3:
    #  a = "Squamous Cell Carcinoma_left Disease detected"
    # elif a==1:
    #   a="Large Cell Carcinoma Disease detected"
    # elif a==0:
    #  a="Adenocarcinoma Disease"
    # else:
    #  a = "Cancer detected."
    return predicted_class

def noufalpredictions(path):
...
class_labels = {
        0: "Adenocarcinoma Disease",
        1: "Large Cell Carcinoma Disease detected",
        2: "No Disease Detected",
        3: "Squamous Cell Carcinoma_left Disease detected",
...
def makepredictions(path):
    ...
    confidence_dict = {class_labels[i]: confidence[i] for i in range(len(confidence))}
    ...
    if a==2:
     ...
     a = "Cancer detected."
    return a,confidence_dict

# # Load model directly
# from transformers import AutoTokenizer, AutoModelForCausalLM

# tokenizer = AutoTokenizer.from_pretrained("Locutusque/gpt2-large-medical")
# model5 = AutoModelForCausalLM.from_pretrained("Locutusque/gpt2-large-medical")
import json
def makepredictions1(path):
    img = Image.open(path)
    ...
    confidence_dict = {class_labels[i]: confidences[i] for i in range(len(confidences))}
    lines = json.dumps(str(confidence_dict), indent = 4)

    return predicted_class_label,max_confidence,lines

def makepredictions2(path):
    img = Image.open(path)
    ...
    output = "Predicted Class: " + predicted_class_label + ": "+ str(max_confidence) + "\n"
    for i in range(len(confidences)):
        class_label = class_labels[i]
        confidence = confidences[i]
        output += f"{class_label}: {confidence} \n"
...
def causes(prompt_1):
  prom = str(prompt_1)
  prompt_2 = "Give me the precautions for this injury. "
  prompt=prompt_2+prom
  input_ids = tokenizer.encode(prompt, return_tensors="pt")
  output = model4.generate(input_ids, max_length=1000, num_return_sequences=1, no_repeat_ngram_size=2, top_k=50)
    ...
    if request.user.is_authenticated and request.user.is_doctor:
        ...
        if request.method == "POST":
            ...
            if f == '':
                ...
                return render(request,'ai.html',{'err':err})
            upload = request.FILES['upload']
            fss = FileSystemStorage()
            file = fss.save(upload.name,upload)
            file_url = fss.url(file)
            predictions = fracture_predict(upload)
            
    # report = causes(predictions)
            return render(request,'ai.html',{'pred':predictions,'doctor':doctor})
        return render(request,'ai.html',{'doctor':doctor})
...
def lungs_precautions(disease):
    ...
    prompt_2 = f"Define this disease that is a type lungs tumor and provide precautionary measures, also dietry precautions for patient for that as well provide full and comprehensive answer. the disase is:  "
    promt = prompt_2 +prom
    # Encode the prompt
    input_ids = tokenizer.encode(promt, return_tensors="pt")
    
    # Generate the output
    output = model4.generate(input_ids, max_length=500, num_return_sequences=1, no_repeat_ngram_size=2, top_k=50)
...
def extract_predicted_class(predict):
    # Use regular expressions to extract the predicted class from the prediction output
    match = re.search(r'Predicted Class: ([\w\s]+)', predict)
    if match:
...
@csrf_exempt
@login_required(login_url="login")
def doctor_ai_view_report(request, pk):
    if request.user.is_authenticated and request.user.is_doctor:
        doctor = Doctor_Information.objects.get(user=request.user)
        report = Report.objects.get(report_id=pk)
        image = report.result
        predict = makepredictions2(image)
        final = extract_predicted_class(predict)
        print(final)
        pred_lines = predict.split('\n')
        precaution = lungs_precautions(final)

        
        context = {'report': report,'doctor': doctor, 'predict':pred_lines,'precaution':precaution}
        return render(request, 'doctor-ai-view-report.html', context)
    else:
        logout(request)
        messages.info(request, 'Not Authorized')
        return render(request, 'doctor-login.html')